...
##### USER INPUT REQUIRED:
ScriptPath=/scratch/jfessler/patientData/shotgunSequencing/fungiLib/FindFungi/scripts #Location of downloaded python and shell scripts
PreDir=/scratch/jfessler/patientData/shotgunSequencing/fungiLib/MyAnalysis #Location you want FindFungi to build the analysis
KrakenDir=/scratch/jfessler/patientData/shotgunSequencing/fungiLib/bioinformatics.czc.hokudai.ac.jp/findfungi/Kraken_32DB  #Location of the 32 downloaded Kraken databases
FungTaxDir=/scratch/jfessler/patientData/shotgunSequencing/fungiLib/FindFungi/Fungal-Taxids  #Location of the Fungal taxids and PipelineSummary files from GitHub
BLAST_DB_Dir=/scratch/jfessler/patientData/shotgunSequencing/fungiLib/bioinformatics.czc.hokudai.ac.jp/findfungi/FungalGenomeDatabases_EqualContigs  #Location of the 949 downloaded BLAST databases
#####
...
for i in $(seq 1 32); do
	kraken --db $KrakenDir/Chunks_$i --threads 30 --fasta-input $PreDir/FASTA/$z.final.fna --output $Dir/Processing/SplitFiles_Kraken/$z.$i &
done	
wait
...
for d in $Dir/Processing/SplitFiles_Kraken/*; do #Gather all reads with a top hit which was fungal 
	File=$(basename $d)
	python2.7 $ScriptPath/KrakenReduction.py $d $Dir/Results/BLAST_Processing/All-Unique-Reads.txt $Dir/Processing/SplitFiles_Kraken/Reduced_${File} &
done
wait
